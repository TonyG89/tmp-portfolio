...
const Header = ({ nameIndex }) => {
    const headerArr = {
        title: ['Too Much Plant Clothes',
            'Storage Too Much Print'],
        desc: ['Речі що є зараз у наявності! За приємними цінами для вас👍',
            'Архів з найкращими нашими роботами']
    }
    const { title, desc } = headerArr

    const changeText = (typeText) => nameIndex ? typeText[1] : typeText[0]
    ...
    const pClass = "text-amber-50 hover:text-amber-300 mt-5 py-1 px-2 w-fit mx-auto rounded-md md:text-md text-base bg-gradient-to-b from-stone-600 to-stone-900"
